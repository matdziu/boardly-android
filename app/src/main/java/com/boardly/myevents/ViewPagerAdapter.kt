...
class ViewPagerAdapter : PagerAdapter() {
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        val pageView = LayoutInflater.from(container.context)
            .inflate(R.layout.view_my_events, container, false)
        val pageEnum = PageView.values()[position]
        when (pageEnum) {
            PageView.ACCEPTED -> init(
                acceptedAdapter,
                R.string.no_accepted_events_found_text,
                pageView,
                pageEnum
            )

            PageView.CREATED -> init(
                createdAdapter,
                R.string.no_created_events_found_text,
                pageView,
                pageEnum
            )

            PageView.PENDING -> init(
                pendingAdapter,
                R.string.no_pending_events_found_text,
                pageView,
                pageEnum
            )

            PageView.INTERESTING -> init(
                interestingAdapter,
                R.string.no_interesting_events_found_text,
                pageView,
                pageEnum
            )
        }
